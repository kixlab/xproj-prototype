<template>
  <div>
    ...
    <div class="ui bottom attached segment">
      <div class="ui cards">
        <promise-item v-for="promise in filteredPromises" :promise="promise" :key="promise.key" :city="city" :district="district" :type="type"></promise-item>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    ...
    city: function () {
      if(!this.promises.city){
        return 'korea'
      } else {
        return this.promises.city
      }
    },
    district: function () {
      if(!this.promises.district){
        return 0
      } else {
        return this.promises.district
      }
    },
