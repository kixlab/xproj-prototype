...
export default new Vuex.Store({
  state: {
    city: 'daejeon',
    district: 3,
    districts: { 'seoul': ['강남구', '강동구', '강북구', '강서구', '관악구', '광진구', '구로구',
  ...
  getters: {
    mayor: state => {
      return state.mayors.find(function (mayor) {
        if (mayor.city === state.city) {
          return mayor
        }
      })
    },
    congressPerson: state => {
      return state.congressPeople.find(function (cp) {
    ...
    mayorPromises: state => {
      return state.mayorsPromises.find(function (mp) {
        if (mp.city === state.city) {
          return mp
        }
      })
    },
    congressPersonPromises: state => {
      return state.congressPromises.find(function (cpp) {
  ...
  mutations: {
    ...
    addComment(state, payload) {
      ...
      console.log(payload)
      if (payload.type === 'president') {
        state.presidentPromises[key].comments.push(payload.comment)
      } else if (payload.type === 'mayor') {
        let promises = state.mayorsPromises.find(function (mp) {
          if(mp.city === payload.city){
            return mp
          }
        })
        promises.promises[key].comments.push(payload.comment)
      } else if (payload.type === 'congressPerson') {
        let promises = state.congressPromises.find(function (cp) {
          if(cp.city === payload.city && cp.district == payload.district){
            return cp
          }
        })
        promises.promises[key].comments.push(payload.comment)
